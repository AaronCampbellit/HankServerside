...
function normalizeOrderedLists() {
  ...
  let blockStartNumber = 1;
  let blockOffset = 0;
  let oldPosition = 0;
  const selectionStart = els.noteContent.selectionStart;
  const selectionEnd = els.noteContent.selectionEnd;
  const changes = [];
  const replacementLines = lines.map((line) => {
    ...
    if (match && (blockIndent === null || blockIndent === match[1])) {
      if (blockIndent === null) {
        blockIndent = match[1];
        blockStartNumber = Number(match[2]);
        blockOffset = 0;
      }
      const expectedNumber = blockStartNumber + blockOffset;
      oldPrefixLength = match[1].length + match[2].length + match[3].length;
      replacement = `${match[1]}${expectedNumber}${match[3]}${match[4]}`;
      newPrefixLength = match[1].length + String(expectedNumber).length + match[3].length;
      blockOffset += 1;
      if (replacement !== line) {
        changed = true;
      }
    } else {
      blockIndent = match ? match[1] : null;
      blockStartNumber = match ? Number(match[2]) : 1;
      blockOffset = match ? 1 : 0;
      if (match) {
        oldPrefixLength = match[1].length + match[2].length + match[3].length;
        newPrefixLength = oldPrefixLength;
      }
    }
    changes.push({ oldStart: oldPosition, oldLine: line, newLine: replacement, oldPrefixLength, newPrefixLength });
  ...
  els.noteContent.value = replacementLines.join("\n");
  els.noteContent.setSelectionRange(
    mapPositionThroughOrderedListChanges(selectionStart, changes),
    mapPositionThroughOrderedListChanges(selectionEnd, changes),
  );
  return true;
}
