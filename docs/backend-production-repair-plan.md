...
Status note, 2026-07-04: waves 0 through 6 are implemented in code and schema. All durable-coordination tables (cloud runtime, agent/app connections, relay requests, app tickets, file transfers, rate limits, login backoff), the deployment-home resolver and authorizer, managed file-operation jobs with per-source policy, durable audit events, metrics/alert rules, lifecycle cleanup, query telemetry, and the validation/load tooling exist and are covered by tests. Remaining work for the first stable release is proof and release discipline, tracked in `RELEASE.md`: run the full demo validation gate against the release candidate and record evidence. Specific line items closed since June: L1 HTTP timeouts are now route-scoped (`routeDeadlineMiddleware`: file transfers get a 30-minute read/write allowance, WebSockets manage their own deadlines, all other routes get tight defaults); L2 placeholder rebinding uses a SQL-aware binder that skips string literals, quoted identifiers, comments, and dollar-quotes, with regression tests; `/metrics` supports a dedicated scrape token (`HANK_REMOTE_METRICS_SCRAPE_TOKEN`) and the compose `monitoring` profile runs Prometheus + Alertmanager against `ops/prometheus/alerts.yml`; release version stamping flows from compose build args through ldflags into `cloud_runtime.version`; `scripts/doctor.sh` now runs the strict plaintext-secret check.
