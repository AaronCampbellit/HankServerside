...
The dashboard model controls do not replace server secrets. Keep OpenAI API keys and ChatGPT/Codex device-code settings in `.env.cloud`; use the GUI to switch between providers, set the Ollama base URL, choose models, choose embedding models, and swap prompt profiles.
...
- **Network (SMB) shares** connect to a NAS or another machine on the network; the agent persists the share list in `HANK_REMOTE_SMB_SHARES_JSON`.
- **Host folders** serve a directory on the home connector itself. The directory may live anywhere the agent can reach on the host and is created on save when "Create folder if it does not exist" is checked. The agent persists host folders in `HANK_REMOTE_AGENT_FILES_ROOTS_JSON` (a JSON array of `{id, name, root, policy}` objects); the legacy single-root `HANK_REMOTE_AGENT_FILES_ROOT` is still read at startup and is folded into that list the first time host folders are saved. Access stays confined to each folder's tree, but the folder itself is not sandboxed to a base path, so only point it at directories you intend to expose.
