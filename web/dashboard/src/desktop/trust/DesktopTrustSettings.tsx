...
export function DesktopTrustSettings({ client = desktopTrustClient, homeID = "", userID = "" }: { client?: DesktopTrustClient; homeID?: string; userID?: string }) {
  ...
  return <section className="settings-panel" aria-label="Remote Desktop trust">
    ...
    {snapshot?.configured&&<section aria-label="Approve Remote Desktop identity"><h3>Approve a device or endpoint</h3><p>Transfer the metadata-only enrollment request to an approved administrator browser, compare its fingerprint out of band, then sign it locally.</p>{currentDeviceRegistered&&localSignerAvailable===false&&<p role="alert">This browser's approved private key is missing. Create a new approval request to re-enroll it.</p>}<button type="button" disabled={localSignerAvailable===null||currentDeviceApproved} onClick={()=>void createOperatorApprovalRequest()}>Create this browser approval request</button><label>Approval request<textarea aria-label="Desktop approval request" value={approvalText} onChange={event=>{setApprovalText(event.target.value);setApproval(null)}}/></label><button type="button" disabled={!approvalText} onClick={()=>void reviewApproval()}>Review approval request</button>{approval&&<DesktopDeviceApproval details={{home:homeID,subject:approvalSubject,platform:approval.request.platform||"unknown",fingerprint:approval.fingerprint,capabilities:approval.request.capabilities,expiry:approval.request.expires_at,signer:currentDeviceID||"approved administrator",changed}} onApprove={()=>void approveReviewed()}/>}</section>}
